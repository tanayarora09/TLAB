...
def dist_wrapper(rank, world_size, func, name: str, SEED: int, lock, shared_list):
    ...
    try:
        func(rank, world_size, name, lock, shared_list)
    finally:
        cleanup_distribute()
...
def main(func, name: str, SEED: int):
    ...
    mp.spawn(dist_wrapper, args=(world_size, func, name, SEED, lock, shared_list), nprocs=world_size, join=True)
