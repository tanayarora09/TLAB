...
if __name__ == '__main__':
    ...
    names = ["rbt", "mbt", "grasp", "grasp_improved", "mse_v1", "imp_vgg16"]#"postmg",]
    titles = {"rbt": "Random", "mgsearch": "GTS with Magnitude", "mbt": "Magnitude", 
              ...
              "mse_v1": "MSE Saliency"}#"postmg": "Magnitude after Training"}

    colors = {"rbt": COLORS["red"], "mgsearch": "blue", "mbt": COLORS["green"], 
              "grasp_improved": COLORS["purple"], "imp_vgg16": COLORS["orange"], 
              "mse_v1": COLORS["blue"], "grasp": COLORS["brown"],}#,"postmg": "pink"}

    results = dict()

    for name in names:

        with open(f"{name}_acc_val.json", "r") as f:
            result = json.load(f)
    ...
    handles.append(plt.axhline(UNPRUNED_MEAN, color = COLORS["black"], linestyle = '-', label = "Unpruned Network"))
